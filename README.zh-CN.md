...
- **中英双语 i18n** -- 默认英文，输入 `lang` 命令可切换为中文。
  全部文案以 `{{var}}` 模板语法存储在 `data/i18n.json`
- **JSON 驱动配置** -- 个人信息、技能、联系方式
  存储在 `data/personal.json`。编辑 JSON，刷新页面即可生效，
  无需修改 JavaScript 代码
- **实时 GitHub 集成** -- 页面加载时自动获取公开仓库列表和个人资料统计
- **Markdown 渲染** -- LLM 回复支持富文本格式：粗体、斜体、
  内联代码、链接、列表和代码块
- **自然语言匹配** -- 可用中英文直接提问，如 "你是谁？"、"你会什么？"、
  "看看你的项目"
- **响应式布局** -- 移动端自适应，字体和边距自动缩小
- **代码块复制** -- 代码块自带"复制"按钮，一键复制到剪贴板
- **OG / Twitter Card** -- 社交媒体分享时显示富媒体预览卡片
- **ARIA 无障碍** -- 语义化属性和标签，兼容屏幕阅读器
- **CSS 自定义属性** -- 设计令牌在 `:root` 中定义，便于主题定制
- **prefers-reduced-motion** -- 操作系统"减少动画"开启时自动跳过动画
- **失败重试** -- 配置文件加载失败时出现"重试"按钮
...
| `lang` | `zh`, `en`, `cn`, `locale` | 切换 / 设置语言（中/英） |
| `clear` | `cls`, `Ctrl+L` / `Cmd+L` | 清屏 |
...
│   └── style.css             # 全部样式
├── js/
│   ├── main.js               # 终端引擎、命令逻辑、LLM 客户端
│   └── markdown.js           # 轻量 Markdown → HTML 解析器
├── data/
│   ├── personal.json         # 可编辑个人信息（姓名、简介、技能、联系方式）
│   └── i18n.json             # 中英文 UI 文案（{{var}} 模板语法）
├── functions/
│   └── api/
...
编辑 [data/personal.json](data/personal.json) -- 无需修改 JavaScript 代码：
...
UI 文案存储在 [data/i18n.json](data/i18n.json)，使用 `{{var}}`
占位符，运行时自动从 `personal.json` 填入对应值。
添加新语言只需复制 `en` 块翻译值，然后在 `js/main.js` 中添加切换逻辑。

### Markdown 样式

编辑 [js/markdown.js](js/markdown.js) 来添加或修改 Markdown 语法。
解析器输出的 HTML 标签映射到 [css/style.css](css/style.css) 中
`.response` 作用域下的 CSS 类（`code`、`pre`、`ul`、`ol`、`li`）。
...
然后将打印的常量代码块复制到 [js/main.js](js/main.js) 中。
脚本输出 `LOGO_UP`、`LOGO_UP_SPLITS`、`LOGO_COLORS`、
`LOGO_UP_WIDTH`、`LOGO_SLANT`、`LOGO_DIR`、`LOGO_ROWS` —
可直接粘贴使用。
...
仓库包含 **GitHub Actions 工作流**（`.github/workflows/deploy-pages.yml`），
每次推送到 `main` 时自动部署到 GitHub Pages。
...
|---|---|
| 标记 | HTML5 |
| 样式 | CSS3（Flexbox、backdrop-filter、自定义属性、自定义滚动条、prefers-reduced-motion） |
...
| 配置 | JSON（`data/personal.json`、`data/i18n.json`） |
| Markdown | 自研零依赖解析器（`js/markdown.js`） |
